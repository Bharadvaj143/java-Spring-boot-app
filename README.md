...
- [Jenkinsfile Reference](#-jenkinsfile-reference)
...
```bash
# Install Docker properly (Ubuntu 24.04)
sudo apt install docker-ce docker-ce-cli containerd.io -y
sudo usermod -aG docker jenkins
sudo chmod 666 /var/run/docker.sock
sudo systemctl restart jenkins
...
### ❌ `docker.io is already the newest version` but Docker not found
> `docker.io` package is broken on Ubuntu 24.04. Use official Docker CE instead (see Part 3).

### ❌ Minikube: `dial unix /var/run/docker.sock: no such file or directory`
```bash
...
### ❌ SonarQube not accessible at port 9000
```bash
# Make sure SonarQube is running
sudo su - sonarqube
cd sonarqube-9.4.0.54424/bin/linux-x86-64/
./sonar.sh status
./sonar.sh start  # if not running
```
Also verify port 9000 is open in EC2 Security Group inbound rules.

### ❌ Jenkins pipeline fails with `exit code 127`
This means a binary is not found. Check:
```bash
